...
public class PackageDBUtil {
    ...
    private static final String INSERT_PKG_SQL = "INSERT INTO packages (PkgName, Description, Price, Duration, ServiceLevel, EventType) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    private static final String UPDATE_PKG_SQL = "UPDATE packages SET PkgName=?, Description=?, Price=?, Duration=?, ServiceLevel=?, EventType=? WHERE ID=?";
    ...
    public void insertPackage(Package pkg) throws SQLException {
        try (Connection conn = DBConnect.getConnection();
            ...
            preparedStatement.setString(5, pkg.getServiceLevel()); // Changed to ServiceLevel
            preparedStatement.setString(6, pkg.getEventType());    // Changed to EventType
    ...
    public Package selectPackage(int id) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            while (rs.next()) {
                ...
                String serviceLevel = rs.getString("ServiceLevel"); // Changed to ServiceLevel
                String eventType = rs.getString("EventType");       // Changed to EventType

                pkg = new Package(id, pkgName, description, price, duration, serviceLevel, eventType);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Package> selectAllPackages() {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            while (rs.next()) {
                ...
                packages.add(new Package(id, pkgName, description, price, duration, serviceLevel, eventType));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
