...
<head>
<meta charset="UTF-8">
<title>Event List</title> <!-- Changed title from "Movie and TV Series List" to "Event List" -->
            ...
            <c:forEach items="${listEvents}" var="event">
                <div class="card">
                    <img src="${pageContext.request.contextPath}/${event.imageUrl}"
                        alt="${event.name}"> <!-- Changed "movie.title" to "event.name" -->
                    <h3>ID: ${event.id}</h3>
                    <p>Name: ${event.name}</p> <!-- Changed "Title" to "Name" -->
                    <p>Type: ${event.type}</p> <!-- Changed "Genre" to "Type" -->
                    <p>Date: ${event.eventDate}</p> <!-- Changed "Release Date" to "Date" -->
